...
DWORD ps_inject(DWORD dwPid, DLL_BUFFER * pDllBuffer)
{
	...
	do
	{
		...
		BREAK_WITH_ERROR("[PS] koadic: dont call this function", ERROR_BAD_ENVIRONMENT);
...
DWORD ps_getarch(DWORD dwPid)
{
	...
	do
	{
		...
		pIsWow64Process = (ISWOW64PROCESS)GetProcAddress((HMODULE)hKernel, "IsWow64Process");
		if (!pIsWow64Process)
			break;
...
DWORD ps_getnativearch(VOID)
{
	...
	do
	{
		...
		pGetNativeSystemInfo = (GETNATIVESYSTEMINFO)GetProcAddress((HMODULE)hKernel, "GetNativeSystemInfo");
