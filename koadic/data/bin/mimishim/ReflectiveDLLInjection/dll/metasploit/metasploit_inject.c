...
*/
DWORD inject_dll(DWORD dwPid, LPVOID lpDllBuffer, DWORD dwDllLenght, DWORD dwReflectiveLoaderOffset, LPVOID lpRemoteCommandLine)
{
	DWORD dwResult = ERROR_ACCESS_DENIED;
	...
	do
	{
		...
		*/
		...
		if (inject_via_remotethread(hProcess, arch, lpReflectiveLoader, lpRemoteCommandLine) != ERROR_SUCCESS)
		{
			...
			if (inject_via_apcthread(hProcess, dwPid, arch, lpReflectiveLoader, lpRemoteCommandLine) != ERROR_SUCCESS)
	...
	return dwResult;//loader_inject_post(dwPid, hProcess, dwResult);
}